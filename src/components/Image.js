...
class Custom extends ImageHandler {
  constructor(provider) {
    super(provider)
    this.uploadUrl = process.env.VUE_APP_Backed_API+'upload'
  }
  makeForm(file, key) {
    console.log(key)
  ...
  urlFromResponse(_, formData) {
    ...
    return `${process.env.VUE_APP_Backed_API}${formData.get('key')}`
  }
}
...
export default class Image extends Node {
  ...
  _upload(file) {
    ...
    try{
      ...
      csrfToken = decodeURIComponent(xsrfCookies[0].split('=')[1]);
